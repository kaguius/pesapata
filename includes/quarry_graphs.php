...
<script type="text/javascript">
      ...
      function drawChart() {
        	var data = google.visualization.arrayToDataTable([
          	...
          	<?php
			$result_tender = mysql_query("SELECT DISTINCT clerk, SUM(invoice_amount)invoice_amount FROM ".$filter_quarry."_quarry.statements WHERE paid_date between '$filter_start_date' and '$filter_end_date' and status = 'Paid' GROUP BY clerk;");
...
<script type="text/javascript">
      ...
      function drawChart() {
        	var data = google.visualization.arrayToDataTable([
          	...
          	<?php
			$result_tender = mysql_query("SELECT DISTINCT product, SUM(invoice_amount)invoice_amount FROM ".$filter_quarry."_quarry.statements WHERE paid_date between '$filter_start_date' and '$filter_end_date' and status = 'Paid' GROUP BY product ORDER BY product ASC");
			while ($row = mysql_fetch_array($result_tender))
			{
				$intcount++;
				$product = $row['product'];
...
<script type="text/javascript">
      ...
      function drawChart() {
        	var data = google.visualization.arrayToDataTable([
          	...
          	<?php
			$result_tender = mysql_query("SELECT DISTINCT paid_date, SUM(invoice_amount)invoice_amount FROM ".$filter_quarry."_quarry.statements where paid_date between '$filter_start_date' and '$filter_end_date' and clerk='$filter_clerk' and status = 'Paid' GROUP BY paid_date order by paid_date asc");
			while ($row = mysql_fetch_array($result_tender))
			{
				$intcount++;
		...
		?>
        	...
        	var chart = new google.visualization.ColumnChart(document.getElementById('quarry_revenue_clerk'));
...
<script type="text/javascript">
      ...
      function drawChart() {
        	var data = google.visualization.arrayToDataTable([
          	...
          	<?php
			$result_tender = mysql_query("SELECT DISTINCT paid_date, SUM(invoice_amount)invoice_amount FROM ".$filter_quarry."_quarry.statements where paid_date between '$filter_start_date' and '$filter_end_date' and product='$filter_market_type' and status = 'Paid' GROUP BY paid_date order by paid_date asc");
			while ($row = mysql_fetch_array($result_tender))
			{
				$intcount++;
				$paid_date = $row['paid_date'];
		...
		?>
        	...
        	var chart = new google.visualization.ColumnChart(document.getElementById('quarry_revenue_product'));
        	chart.draw(data, options);
