      ...
      function drawChart() {
			...
			while ($row = mysql_fetch_array($result_tender))
			{
				...
				?>
					  ...
					  ['<?php echo $clerk ?>',  'dashboard.php?market=<?php echo $filter_market ?>&clerk=<?php echo $clerk ?>&report_start_date=<?php echo $filter_start_date ?>&report_end_date=<?php echo $filter_end_date ?>', <?php echo $invoice_amount?>, '<?php echo $invoice_amount?>'],
				<?php
			}
		?>
        ]);
      ...
      function drawChart() {
			...
			while ($row = mysql_fetch_array($result_tender))
			{
				...
				?>
					...
					['<?php echo $product ?>', 'dashboard.php?market=<?php echo $filter_market ?>&market_type=<?php echo $product ?>&report_start_date=<?php echo $filter_start_date ?>&report_end_date=<?php echo $filter_end_date ?>', <?php echo $invoice_amount?>, '<?php echo $invoice_amount?>'],
				<?php
			}
		?>
        ]);
...
<script type="text/javascript">
      ...
      function drawChart() {
        	var data = google.visualization.arrayToDataTable([
          	...
          	<?php
			$result_tender = mysql_query("SELECT DISTINCT paid_date, SUM(invoice_amount)invoice_amount FROM ".$filter_market."_market_fees.statements where paid_date between '$filter_start_date' and '$filter_end_date' and clerk='$filter_clerk' and status = 'Paid' GROUP BY paid_date order by paid_date asc");
			while ($row = mysql_fetch_array($result_tender))
			{
				$intcount++;
